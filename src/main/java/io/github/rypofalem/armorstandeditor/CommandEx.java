...
public class CommandEx implements CommandExecutor {
	...
	private void commandSlot(final Player player, final String[] args) {
		...
		if (args.length > 1) {
			try {
				final byte slot = (byte) (Byte.parseByte(args[1]) - 0b1);
				if (slot >= 0 && slot < 9) {
	...
	private void commandAdj(final Player player, final String[] args) {
		if (args.length <= 1) {
		...
		if (args.length > 1) {
			for (final AdjustmentMode adj : AdjustmentMode.values()) {
	...
	private void commandAxis(final Player player, final String[] args) {
		if (args.length <= 1) {
	...
	private void commandMode(final Player player, final String[] args) {
		if (args.length <= 1) {
	...
	private void commandHelp(final Player player) {
	...
	private boolean checkPermission(final Player player, String permName, final boolean sendMessageOnInvalidation) {
